...
function SignIn() {
  ...
  const handleLogin = async (event) => {
    ...
    try {
      ...
      if (users.length > 0) {
        // Here you would handle the session (e.g., setting local storage, state management, etc.)
  ...
  return (
    <>
      <div className='mt-40 flex  justify-center  bg-darkBlue flex mx-auto space-y-1 h-auto w-2/5 rounded-lg'>
        <form onSubmit={handleLogin} className='space-y-6 flex flex-col justify-center items-center h-full w-full' action="" method="post">
            <br />
          <h1 className='text-4xl px-5 py-1 text-white pb-5'>Se connecter</h1>
          <input className='mt-5 w-2/3 p-2 rounded-full' 
                  type="email" 
                  name="email" 
                  id="email" 
                  placeholder="Email" 
          ...
          <button className='my-10 w-1/5 p-2 font-bold rounded-full text-darkBlue bg-white hover:text-green-500' type="submit">Sign In</button>
          {error && <div className='text-red-500'>{error}</div>}
          <div className='flex flex-row justify-center space-x-5'>
            <p className='text-white'>Premiere inscription ? </p> 
            <a className='text-white font-bold' href="/signup"> Créer un compte</a>
          </div>
          <br /><br />
        </form>    
      </div>
